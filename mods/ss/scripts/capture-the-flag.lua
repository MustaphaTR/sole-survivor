	...
	Trigger.AfterDelay(0, function()
		if CtFOption ~= "disabled" then
			for _,player in pairs(players) do
				if not Units[player.InternalName].IsDead and Units[player.InternalName].Location == FlagCircles[player.InternalName].Location then
					local flag = Units[player.InternalName].DropFlag()
					if flag ~= nil then
						if flag.Owner ~= player then
							player.Experience = player.Experience + 25
							if CtFOption == "score" then
								flag.Teleport(FlagCircles[flag.Owner.InternalName].Location)
							else
								flag.Destroy()
								flag.Owner.MarkFailedObjective(0)
							end
						end
					end
				end
